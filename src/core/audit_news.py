...
def get_news_by_id(news_id):
    ...
    with get_db() as conn:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, title, title_chs, news_url, article_content, content_summary FROM news WHERE id = ?", (news_id,)
        )
        row = cursor.fetchone()
    return row
...
def run_audit_one(llm_type=None):
    ...
    if need_summary or not content_summary.strip():
        ...
        summary_choice = input("请选择 [Enter/m]: ").strip().lower()

        if summary_choice == "m":
            print("请输入中文摘要 (--END-- 单独一行结束):")
        ...
        if summary_choice != "m" or not final_summary.strip():
            if final_content.strip():
                ...
                if final_summary:
                    ...
                    manual2 = multiline_input("请输入中文摘要 (--END-- 结束):")
    ...
    if final_summary and final_summary.strip():
        ...
        final_title = input("请手动输入中文标题: ").strip()
