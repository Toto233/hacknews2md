...
def is_gemini_quota_exceeded_error(error_msg):
    ...
    has_exceeded_semantics = any(marker in msg for marker in ["exceed", "exceeded", "limit", "quota", "resource_exhausted"])
        ...
        with self.lock:
            ...
            for _ in range(len(self.models)):
                ...
                if is_model_disabled_today('gemini', model):
                    print(f"[配额熔断] 跳过今日禁用模型: {model}")
                    continue
                print(f"[负载均衡] 选择模型: {model} (索引: {idx}/{len(self.models)})")
                return model
            print("[配额熔断] Gemini 候选模型今日均不可用")
            return None
...
def call_gemini_api(prompt, model=None, temperature=None, max_tokens=None, response_format=None, max_retries=5, image_data=None):
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            print("错误: 新版 google-genai SDK 未安装")
