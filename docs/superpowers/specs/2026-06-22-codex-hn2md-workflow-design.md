...
`CollectStage` 接收 `concurrency`，复用当前 Codex collector 的行为：
...
`hn2md plan` 新增互斥选项 `--manual-plan PATH`。传入时：
...
`RenderStage` 从 APPLYING receipt 获取同一 plan，严格按照 `ordered_ids` 渲染，并使用 plan 中的四个 tags。渲染产物包括 Markdown、HTML 和可选 Astro Markdown，路径全部写入 receipt。渲染逻辑迁入可复用模块，旧 argparse 脚本只作为兼容包装或停止被 skill 引用。
