...
def display_html_in_browser(html_content: str, auto_close: bool = True, close_delay: int = 5):
    ...
    try:
        ...
        if auto_close:
            logger.info(f"HTML已在浏览器中打开，{close_delay}秒后自动关闭...")
            ...
            try:
                ...
                subprocess.run(["taskkill", "/f", "/im", "firefox.exe"], capture_output=True, check=False)
            except Exception:
                pass
        else:
            logger.info("HTML已在浏览器中打开，请手动关闭浏览器标签页")
...
def _cli_main():
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(html_content)
    logger.info(f"已生成HTML文件: {output_path}")
    ...
    if not args.no_open:
        ...
        if args.auto_close is not None and args.auto_close > 0:
            ...
            if bm_display:
                ...
                logger.info("HTML已在浏览器中打开（简单模式），请手动关闭浏览器标签页")
