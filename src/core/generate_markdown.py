...
# Astro 博客项目路径 (from env, empty string = disabled)
ASTRO_PROJECT_DIR = os.environ.get("HACKNEWS_ASTRO_REPO", "")
ASTRO_BLOG_DIR = os.path.join(ASTRO_PROJECT_DIR, "src", "data", "blog") if ASTRO_PROJECT_DIR else ""
ASTRO_PUBLIC_IMAGES_DIR = os.path.join(ASTRO_PROJECT_DIR, "public", "images") if ASTRO_PROJECT_DIR else ""
...
def cleanup_old_astro_images(astro_images_dir, days=10):
    ...
    for name in os.listdir(astro_images_dir):
        ...
        if re.match(r"^\d{8}$", name) and name < cutoff_str:
            ...
            logger.info(f"Cleaned up old Astro images: {dir_path}")
...
def generate_markdown():
    ...
    # Sanitize titles to prevent YAML injection
    safe_title_chs = sanitize_for_yaml(first_title_chs) if first_title_chs else ""
    safe_title = sanitize_for_yaml(first_title) if first_title else ""
    prefix = safe_title_chs if safe_title_chs else safe_title
    ...
    # Sanitize digest to prevent YAML injection
    safe_digest = sanitize_for_yaml(first_content_summary[:120]) if first_content_summary else ""
    yaml_header = f"""---
title: {yaml_quote(yaml_title)}
author: {yaml_quote("hacknews")}
description: {yaml_quote("")}
digest: {yaml_quote(safe_digest)}
...
"""
    ...
    for idx, (
        ...
        image_3,
        screenshot,
    ) in enumerate(sorted_news_items, 1):
        markdown_content += "---\n\n"

        # 标题部分：中文标题(英文标题)
        # Sanitize titles to prevent injection in markdown content
        safe_title_chs = sanitize_for_yaml(title_chs) if title_chs else ""
        safe_title = sanitize_for_yaml(title) if title else ""
        display_title = f"{safe_title_chs} ({safe_title})" if safe_title_chs else safe_title
    ...
    logger.info(f"Successfully generated markdown file: {md_filename}")
    ...
    logger.info(f"Successfully generated HTML file: {html_filename}")

    # --- 生成 Astro 博客版本 ---
    if ASTRO_PROJECT_DIR:
        # 收集所有截图路径（仅截图，不含正文大图）
        ...
        logger.info(f"Successfully generated Astro blog file: {astro_md_filename}")
    else:
        logger.info("HACKNEWS_ASTRO_REPO not set, skipping Astro blog generation")
    ...
    try:
        ...
        logger.info("已复制标题到剪贴板")
    except ImportError:
        logger.warning("pyperclip not installed, skipping clipboard copy")
    logger.info(f"使用以下命令发布到微信公众号: python scripts/publish_wechat.py \"{md_filename}\"")
